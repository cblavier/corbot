    ...
    context "with an admin user" do
      ...
      context "with some unbound users" do
        ...
        it "should contain a block with the count of unbound users" do
          expect(subject["blocks"]).to satisfy do |blocks|
            blocks_contain?(blocks, "section", text: "Voici le premier membre des \\*#{unbound_users.count}\\* dont le compte Slack est inconnu :")
          end
        end

        it "should contain a block with a user_select list" do
          expect(subject["blocks"]).to satisfy do |blocks|
            blocks_contain?(blocks, "section", text: "Indiquer quel est le compte Slack de")
          end
        end
      end
    end
